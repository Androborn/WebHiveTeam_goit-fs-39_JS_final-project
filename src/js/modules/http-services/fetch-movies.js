...
export class fetchMovies {
    ...
    async getMovies() {
        try {
            const response = await axios.get(`/3/trending/all/day?api_key=${this.API_KEY}`)
    ...
    async getMoviesByKeyword() {
        try {
            const response = await axios.get(`/3/search/movie?api_key=${this.API_KEY}&query=${this.keyword}`)
            const data = await response.data;
            return data;
        }
        catch (error) {
